...
/**
 * @function: void OLED_ShowInt(uint8_t x,uint8_t y,int16_t num,uint8_t len,uint8_t size2,uint8_t Color_Turn)
 * @description: 显示正负整数
 * @param {uint8_t} x待显示的数字起始横坐标,x:0~126
 * @param {uint8_t} y待显示的数字起始纵坐标, y:0~7，若选择字体大小为16，则两行数字之间需要间隔2，若选择字体大小为12，间隔1
 * @param {int16_t} num:输入的整数数据（支持负数）
 * @param {uint8_t } len:输入的数据位数（不包括正负号）
 * @param {uint8_t} size2:输入的数据大小，选择 16/12，16为8X16，12为6x8
 * @param {uint8_t} Color_Turn是否反相显示(1反相、0不反相)
 * @return {*}
 */
void OLED_ShowInt(uint8_t x,uint8_t y,int16_t num,uint8_t len,uint8_t size2,uint8_t Color_Turn)
{
	uint8_t t, temp, sign_flag = 0;
	uint8_t enshow = 0;
	unsigned int abs_num;
	uint8_t total_width = len + 1; // 包括正负号位
	...
	// 显示整数部分
	for(t = 1; t < total_width; t++)
	{
		uint8_t digit_index = t - 1;
		temp = (abs_num / oled_pow(10, len - digit_index - 1)) % 10;

		if(enshow == 0 && digit_index < (len - 1))
		{
			if(temp == 0)
			{
				OLED_ShowChar(x + (size2 / 2) * t, y, ' ', size2, Color_Turn);
...
void OLED_Showdecimal(uint8_t x,uint8_t y,float num,uint8_t z_len,uint8_t f_len,uint8_t size2, uint8_t Color_Turn)
{
	...
	total_int = z_len + 1; // 一位留给正负号或空格
	z_temp = (int)num;

	// 整数部分（包括符号位）
	for(t = 0; t < total_int; t++)
	{
		if(t == 0)
		{
			if(sign_flag)
		...
		uint8_t digit_index = t - 1;
		temp = (z_temp / oled_pow(10, z_len - digit_index - 1)) % 10;
		if(enshow == 0 && digit_index < (z_len - 1))
		{
			if(temp == 0)
			{
				OLED_ShowChar(x + (size2 / 2) * t, y, ' ', size2, Color_Turn);
	...
	// 小数点
	OLED_ShowChar(x + (size2 / 2) * total_int, y, '.', size2, Color_Turn);
	...
	for(t = 0; t < f_len; t++)
	{
		...
		OLED_ShowChar(x + (size2 / 2) * (total_int + 1 + t), y, temp + '0', size2, Color_Turn);
	}
}
